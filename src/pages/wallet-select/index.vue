<template>
  <q-page>
    <div class="welcome-title">{{ $t("titles.welcomeToJude") }}</div>
    <q-list class="wallet-list" link no-border :dark="theme == 'dark'">
              ...
              <q-list separator>
                ...
                >
                  <q-item-section avatar>
                    <q-icon :name="action.icon" color="primary" size="24px" />
                  </q-item-section>
                  <q-item-section>
                    {{ action.name }}
                  </q-item-section>
      ...
      <template v-else>
        ...
        >
          <q-item-section avatar>
            <q-icon :name="action.icon" color="primary" size="2.2rem" />
          </q-item-section>
          <q-item-section>
            {{ action.name }}
          </q-item-section>
...
export default {
  ...
  computed: mapState({
    ...
    actions() {
      ...
      const actions = [
        ...
        {
          name: this.$t("titles.wallet.restoreFromKeys"),
          icon: "vpn_key",
          handler: this.restoreWalletFromKeys
        },
        {
  ...
  methods: {
    ...
    restoreWalletFromKeys() {
      this.$router.replace({ path: "wallet-select/restore-keys" });
    },
    restoreViewWallet() {
